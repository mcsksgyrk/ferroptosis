...
class PsimiSQL:
    ...
    def insert_node(self, node_dict):
        ...
        if ('id' not in node_dict) and not existing_node:
            ...
            query = """
                INSERT INTO node
                (name, display_name, tax_id, type, pathways, source, function)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """
    ...
    def get_node_by_id(self, id):
        ...
        if not answer:
            ...
            # Adjust to the new schema with display_name and type fields
            id, name, display_name, display_name, tax_id, mol_type, pathways, source, function = answer
            node_dict = {
                "id": id,
                "name": name,
    ...
    def update_node(self, node_dict):
        ...
        query = """
            UPDATE node
            SET name = ?, display_name = ?, tax_id = ?, type = ?,
    ...
    def insert_node_identifier(self, node_id, id_type, id_value):
        """Insert an identifier for a node"""
        query = "INSERT INTO node_identifier (node_id, id_type, id_value) VALUES (?, ?, ?)"
        try:
            self.cursor.execute(query, (node_id, id_type, id_value))
            self.db.commit()
        except sqlite3.IntegrityError:
            # If already exists, update instead
            self.update_node_identifier(node_id, id_type, id_value)

    def update_node_identifier(self, node_id, id_type, id_value):
        ...
        if id_value and id_value != '-':
            self.cursor.execute("INSERT INTO node_identifier (node_id, id_type, id_value) VALUES (?, ?, ?)",
                               (node_id, id_type, id_value))
        self.db.commit()
